...
local tagStrings = {
	...
	['perhp'] = [[function(u)
		...
		if m == 0 then
			...
			return math.floor(UnitHealth(u) / m * 100 + 0.5)
		end
	end]],
	...
	['perpp'] = [[function(u)
		...
		if m == 0 then
			...
			return math.floor(UnitPower(u) / m * 100 + 0.5)
		end
	end]],
...
eventFrame:SetScript('OnEvent', function(self, event, unit)
	...
	if strings then
		for _, fs in next, strings do
			if
				fs:IsVisible()
				and (unitlessEvents[event] or fs.parent.unit == unit or (fs.extraUnits and fs.extraUnits[unit]))
			then
				fs:UpdateTag()
			end
		end
