...
local UNIT_SPELLCAST_START = function(self, event, unit, spell, spellrank)
	...
	local name, rank, text, texture, startTime, endTime, _, castid, interupt = UnitCastingInfo(unit)
	if(not name) then
		castbar:Hide()
		return
	end
	...
	if(self.PostCastStart) then self:PostCastStart(event, unit, name, rank, text, castid, interupt) end
	castbar:Show()
end
...
local UNIT_SPELLCAST_CHANNEL_START = function(self, event, unit, spellname, spellrank)
	...
	local castbar = self.Castbar
	local name, rank, text, texture, startTime, endTime, isTrade, interupt = UnitChannelInfo(unit)
	if(not name) then
		return
	end
	...
	if(self.PostChannelStart) then self:PostChannelStart(event, unit, name, rank, text, interupt) end
	castbar:Show()
end
...
local UNIT_SPELLCAST_CHANNEL_UPDATE = function(self, event, unit, spellname, spellrank)
	...
	if(self.PostChannelUpdate) then self:PostChannelUpdate(event, unit, name, rank, text, castbar.interupt) end
end
